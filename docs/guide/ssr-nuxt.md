...
```
add the plugin to your `nuxt.config.js` file as shown below : 
...
- Dehydrate `queryClient` to the nuxt context.
...
As demonstrated, it's possible to selectively prefetch some queries while letting others fetch on the `queryClient`. This gives you control over what content the server renders by adding or removing the `prefetchQuery` or `suspense` for a specific query.
...
In some cases, this behavior may not be desirable. For example, you may want to render an error page with a correct status code for certain errors or queries. In such cases, you can use `fetchQuery` and catch any errors to handle them manually.

### Staleness is measured from when the query was fetched on the server

A query is considered stale depending on when its data was last updated `(dataUpdatedAt)`. A caveat here is that the server needs to have the correct time for this to work properly. However, UTC time is used, so timezones do not affect this.
